...
export const receiveTopics = inputedValue => dispatch => {
    dispatch({
        type: GET_TOPICS_REQUEST,
        payload: {error: false, isLoading: true},
    });
    axios
        .get(`https://api.github.com/search/topics?q=react+is:featured`,
            {headers: {Accept: 'application/vnd.github.mercy-preview+json'}})
        .then(response => {
            dispatch({
                type: GET_TOPICS_SUCCESS,
                payload: {error: false, isLoading: false, data: response.data[0] }
            });
